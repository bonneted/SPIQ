...
X_dic = pd.read_csv(f"{dic_path}/X_trans/pattern_2MP_Numerical_9_0.synthetic.tif_X_trans.csv", delimiter=";").dropna(axis=1).to_numpy() 
Y_dic = pd.read_csv(f"{dic_path}/Y_trans/pattern_2MP_Numerical_9_0.synthetic.tif_Y_trans.csv", delimiter=";").dropna(axis=1).to_numpy() 
U_dic = pd.read_csv(f"{dic_path}/U_trans/pattern_2MP_Numerical_9_0.synthetic.tif_U_trans.csv", delimiter=";").dropna(axis=1).to_numpy().T.reshape(-1,1)
V_dic = pd.read_csv(f"{dic_path}/V_trans/pattern_2MP_Numerical_9_0.synthetic.tif_V_trans.csv", delimiter=";").dropna(axis=1).to_numpy().T.reshape(-1,1)
...
measure_U = dde.PointSetOperatorBC(X_DIC_input, U_dic, lambda x, f, x_np: f[0][:, 0:1])
measure_V = dde.PointSetOperatorBC(X_DIC_input, V_dic, lambda x, f, x_np: f[0][:, 1:2])

bcs = []
num_boundary = 0

if n_DIC:
    bcs += [measure_U, measure_V]
...
folder_name = f"DIC_{net_type}_E-{E_init}_nu-{nu_init}_nDIC-{n_DIC**2}_noise-{noise_ratio}_{available_time if available_time else n_iter}{'min' if available_time else 'iter'}"
