"""
The idea is to use strain as DIC data to corrupt with 1e-6 magnitude gaussian noise the same way as Martin et al. (2019)
It required to modify dde.data.PDE so that pure function is passed to the PointSetOperatorBC, allowing the computation of the strain from the output (using jacobian)
"""
...
strain_interp = []
for i in range(strain_val.shape[1]):
    interp = RegularGridInterpolator((x_grid, y_grid), strain_val[:, i].reshape(n_mesh_points, n_mesh_points).T)
...
def strain_fn(x):
    ...
    return np.array([interp((x[:,0], x[:,1])) for interp in strain_interp]).T
...
folder_name = f"{net_type}_E-{E_init}_nu-{nu_init}_nDIC-{n_DIC**2}_noise-{noise_magnitude}_{available_time if available_time else n_iter}{'min' if available_time else 'iter'}"
